package com.bridgelabz.stream.main;
...
public class NumberPlaylistClass
{
    public static void main(String[] args) {
        ...
        Integer evenFirst =  myNumberList.stream()
                .filter(isEven)
                .peek(integer -> System.out.println("peek even No first even No "+integer))
                .findFirst()
                .orElse(null);
        ...
        Integer min = myNumberList.stream().filter(isEven)
                .min((n1,n2)->n1-n2)
                .orElse(null);
        System.out.println("Min even no is "+min);

        //method 12 to get max even no
        Integer max = myNumberList.stream().filter(isEven)
                .max((n1,n2)->n1-n2)
                .orElse(null);
        System.out.println("Max even no is "+max);
